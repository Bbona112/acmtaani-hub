...
export default function Auth() {
  ...
  const desc = mode === 'login' ? 'Sign in to ACMtaani Hub' : mode === 'signup' ? 'Get started with ACMtaani Hub' : 'Enter your email to receive a reset link';
  const btn = mode === 'login' ? 'Sign in' : mode === 'signup' ? 'Sign up' : 'Send reset link';
      ...
      <Card className="w-full max-w-md shadow-lg border-border/50">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && <Input placeholder="Full name" value={fullName} onChange={e => setFullName(e.target.value)} required />}
            <Input type="email" placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)} required />
            {mode !== 'forgot' && (
              <Input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} required minLength={6} />
            )}
            <Button type="submit" className="w-full" disabled={loading}>{loading ? 'Please wait...' : btn}</Button>
            {mode === 'login' && (
              <button type="button" onClick={() => setMode('forgot')} className="text-xs text-muted-foreground hover:underline w-full text-center">
          ...
          <p className="mt-4 text-center text-sm text-muted-foreground">
            {mode === 'login' && (<>Don't have an account?{' '}<button onClick={() => setMode('signup')} className="text-primary font-medium hover:underline">Sign up</button></>)}
            {mode === 'signup' && (<>Already have an account?{' '}<button onClick={() => setMode('login')} className="text-primary font-medium hover:underline">Sign in</button></>)}
            {mode === 'forgot' && (<button onClick={() => setMode('login')} className="text-primary font-medium hover:underline">Back to sign in</button>)}
          </p>
        </CardContent>
      </Card>
    </div>
