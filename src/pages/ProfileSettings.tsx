...
export default function ProfileSettings() {
  ...
  const printBadge = useCallback(() => {
    ...
    pw.document.write(`<html><head><title>Badge</title><style>@page{size:3.375in 2.125in;margin:0}body{margin:0;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:system-ui,sans-serif}.badge{width:3.375in;height:2.125in;border:1px solid #ddd;border-radius:8px;overflow:hidden;background:#fff}.header{background:#1a1a2e;color:#fff;padding:8px 12px;font-size:10px;font-weight:bold}.body{display:flex;padding:10px 12px;gap:10px}.info{flex:1}.name{font-size:14px;font-weight:bold;margin-bottom:4px}.position{font-size:11px;color:#4444cc;font-weight:bold;margin-bottom:2px}.detail{font-size:9px;color:#666;margin-bottom:2px}.qr img{width:90px;height:90px}.footer{text-align:right;padding:0 12px 6px;font-size:7px;color:#999}</style></head><body><div class="badge"><div class="header">ACMtaani Hub</div><div class="body"><div class="info"><div class="name">${esc(profile?.full_name)}</div><div class="position">${esc(profile?.position || 'Member')}</div><div class="detail">${esc(profile?.department)}</div><div class="detail">${esc(profile?.employee_id)}</div><div class="detail" style="margin-top:6px">${esc(profile?.email)}</div></div><div class="qr"><img src="${qrDataUrl}" /></div></div><div class="footer">Scan QR to clock in/out</div></div></body></html>`);
